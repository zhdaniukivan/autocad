...
class AutoCADHandler:
    ...
    def connect(self):
        ...
        # 1. Подключаемся к уже запущенному экземпляру AutoCAD
        try:
            self.acad = win32com.client.GetActiveObject("AutoCAD.Application")
            self.doc = self.acad.ActiveDocument
            logging.info("✅ Подключено к уже запущенному AutoCAD")
            return True
        except Exception as e:
            logging.debug(f"GetActiveObject не удался: {e}")

        # 2. Пытаемся запустить новый экземпляр AutoCAD
        try:
            self.acad = win32com.client.Dispatch("AutoCAD.Application")
            # Безопасно устанавливаем Visible
            try:
                if hasattr(self.acad, 'Visible') and not self.acad.Visible:
                    self.acad.Visible = True
            except Exception as ve:
                logging.warning(f"Не удалось установить Visible: {ve}")
            self.doc = self.acad.ActiveDocument
            logging.info("✅ AutoCAD успешно запущен через Dispatch")
            return True
        except Exception as e:
            logging.error(f"❌ Не удалось подключиться/запустить AutoCAD: {e}")
            self.acad = None
            self.doc = None
            return False

    def is_connected(self):
        """Проверка, успешно ли подключены к AutoCAD"""
        return self.acad is not None and self.doc is not None

    # ---------- Получение выделенных блоков ----------
    def get_selected_blocks(self):
        if not self.is_connected():
            logging.error("Попытка получения блоков без подключения к AutoCAD")
            return []
        try:
    ...
    # ---------- Чтение/запись динамических свойств ----------
    ...
    def apply_target_width_bottom(self, blocks, target_width, regen=True):
        if not self.is_connected():
            return False, 0, "Нет подключения к AutoCAD"
    ...
    def apply_overall_height(self, blocks, target_opening_height, regen=True):
        if not self.is_connected():
            return False, "Нет подключения к AutoCAD"
