...
class BlockEditorGUI:
    def __init__(self):
        ...
        # Проверка подключения к AutoCAD
        if not self.handler.is_connected():
            messagebox.showerror(
                "Ошибка подключения к AutoCAD",
                "Не удалось подключиться к AutoCAD.\n\n"
                "Пожалуйста:\n"
                "1. Запустите AutoCAD\n"
                "2. Откройте нужный чертёж\n"
    ...
    def create_widgets(self):
        ...
        self.tree.heading('changeD', text='ChangeHeight')
        ...
        # Ширина (одно поле для обеих линий)
        width_frame = tk.Frame(group_ctrl, bg='#1e1e1e')
        width_frame.pack(fill='x', padx=10, pady=8)
        tk.Label(width_frame, text="Целевая общая ширина (для верхней и нижней линии):",
                 bg='#1e1e1e', fg='white', width=45, anchor='w').pack(side='left')
        self.target_width_var = tk.StringVar()
        ttk.Entry(width_frame, textvariable=self.target_width_var, width=12).pack(side='left', padx=10)
        self.apply_width_btn = tk.Button(width_frame, text="📏 Применить ширину",
                                         command=self.apply_width_both,
                                         bg='#2e7d32', fg='white')
        self.apply_width_btn.pack(side='left', padx=5)

        # Высота
        h_frame = tk.Frame(group_ctrl, bg='#1e1e1e')
        h_frame.pack(fill='x', padx=10, pady=8)
        tk.Label(h_frame, text="Целевая общая высота (Opening_Height):",
                 bg='#1e1e1e', fg='white', width=45, anchor='w').pack(side='left')
        ...
        self.apply_all_btn = tk.Button(all_frame, text="🚀 Применить все (ширина + высота)",
        ...
        # Скроллируемая область для свойств одного блока
    ...
    def validate_blocks(self):
        ...
        if warnings:
            ...
            messagebox.showinfo("Проверка блоков", "✅ Все блоки соответствуют логике!")
    ...
    def update_special_elements(self):
        if not self.current_blocks:
            messagebox.showinfo("Нет блоков", "Сначала обновите выделение из AutoCAD")
            return
        result = self.handler.update_special_elements(self.current_blocks, regen=True)
        if result['updated']:
    ...
    def reset_group(self):
        ...
        self.pure_bottom_label.config(text="🧩 Чистые нижние: 0.0 мм")
        self.pure_top_label.config(text="🧩 Чистые верхние: 0.0 мм")
    ...
    def update_statistics(self):
        ...
        full_count = sum(1 for b in self.current_blocks if self.handler.get_block_type(b) == 'full')
        ...
        if opening_h:
            self.update_status(f"📐 Общая высота проёма: {opening_h:.1f} мм")

    # ---------- Применение ширины к обеим линиям ----------
    def apply_width_both(self):
        if not self.current_blocks:
            messagebox.showinfo("Нет блоков", "Сначала обновите выделение из AutoCAD")
            return
        try:
            target = float(self.target_width_var.get())
        except:
            messagebox.showerror("Ошибка", "Введите число для целевой ширины")
            return

        blocks = self.current_blocks[:]
        start_time = time.time()
        self.update_status("📏 Применение ширины...")
        self.root.update()

        # Запоминаем исходный левый край
        min_x_before, _, _ = self.handler.get_bottom_blocks_x_range(blocks)

        success_bottom, _, msg_bottom = self.handler.apply_target_width_bottom(blocks, target, regen=False)
        success_top, _, msg_top = self.handler.apply_target_width_top(blocks, target, regen=False)

        # Регенерация и выравнивание
        self.handler.doc.Regen(1)
        if min_x_before is not None:
            self.handler.align_bottom_line_left(blocks, min_x_before)

        # Обновление спецэлементов
        self.handler.update_special_elements(blocks, regen=True)

        self.refresh_selection()
        elapsed = time.time() - start_time
        self.update_status(f"✅ Ширина применена за {elapsed:.1f} сек\n{msg_bottom}\n{msg_top}")
    ...
    def apply_overall_height(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", "Введите число для целевой высоты")
            return

        start_time = time.time()
        success, msg = self.handler.apply_overall_height(self.current_blocks, target, regen=True)
        elapsed = time.time() - start_time
        if success:
            self.refresh_selection()
            self.update_status(f"✅ Высота изменена за {elapsed:.1f} сек\n{msg}")
        else:
    ...
    # ---------- Пакетное применение (ширина → высота) с таймером ----------
    def apply_all_operations(self):
        ...
        width_val = self.target_width_var.get().strip()
        height_val = self.target_height_var.get().strip()

        start_time = time.time()
        any_success = False

        # Сохраняем исходный левый край нижней линии (на случай смещения)
        min_x_before, _, _ = self.handler.get_bottom_blocks_x_range(blocks)

        # ========== 1. ШИРИНА ==========
        if width_val:
            try:
                target = float(width_val)
                self.update_status(f"📏 Ширина: {target} мм...")
                self.root.update()

                # Применяем ширину без регенерации
                success_bottom, _, msg_bottom = self.handler.apply_target_width_bottom(blocks, target, regen=False)
                success_top, _, msg_top = self.handler.apply_target_width_top(blocks, target, regen=False)

                if success_bottom or success_top:
                    any_success = True
                    self.update_status(f"{msg_bottom}\n{msg_top}")

                    # Регенерация
                    self.handler.doc.Regen(1)
                    self.update_status("⏳ Регенерация после ширины...")
                    self.root.update()
                    ...
                    if min_x_before is not None:
                        self.handler.align_bottom_line_left(blocks, min_x_before)
                        self.update_status("🔧 Выравнивание левого края после ширины...")
                        self.root.update()
                        time.sleep(0.1)

                    # Обновление спецэлементов после ширины
                    self.update_status("🔄 Обновление спецэлементов после ширины...")
                    self.handler.update_special_elements(blocks, regen=True)
                    self.root.update()
                    time.sleep(0.1)
                else:
                    self.update_status("⚠️ Ширина не изменена")
            except ValueError:
                self.update_status("❌ Ширина: не число")
        time.sleep(0.5)
        # ========== 2. ВЫСОТА ==========
        if height_val:
            try:
                ...
                # Применяем высоту без регенерации
                success, msg = self.handler.apply_overall_height(blocks, target, regen=False)
                if success:
                    any_success = True
                    ...
                    # Регенерация
                    self.handler.doc.Regen(1)
                    self.update_status("⏳ Регенерация после высоты...")
                    self.root.update()


                    # Повторное выравнивание левого края (на случай, если высота повлияла на X)
                    if min_x_before is not None:
                        self.handler.align_bottom_line_left(blocks, min_x_before)
                        self.update_status("🔧 Повторное выравнивание левого края...")
                        self.root.update()


                    # Обновление спецэлементов после высоты
                    self.update_status("🔄 Обновление спецэлементов после высоты...")
                    self.handler.update_special_elements(blocks, regen=True)
                    self.root.update()
        ...
        if any_success:
            self.update_status("🔄 Финальная регенерация и синхронизация...")
            self.handler.doc.Regen(1)
            time.sleep(0.5)
            self.refresh_selection()
        else:
        ...
        # Жёлтый цвет при превышении 30 секунд
        if elapsed >= 30:
            self.update_status(f"⚠️ Пакетное изменение завершено за {elapsed:.1f} сек (превышает 30 сек)")
            self.status_label.config(fg='#ffcc00')
            self.root.after(3000, lambda: self.status_label.config(fg='#66ff99'))
        else:
    ...
    def toggle_all_params(self):
        ...
        self.toggle_all_params_btn.config(
            text="📋 Скрыть все параметры" if self.single_all_params_visible else "📋 Показать все параметры"
        )
        self.build_single_editor()
    ...
    def build_single_editor(self):
        ...
        type_display = {
            'bottom': 'Нижний', 'top': 'Верхний', 'full': 'Полной высоты (низ+верх)',
            'special': 'HeightsForCabs (специальный)', 'unknown': 'Неизвестный'
        }.get(block_type, '?')
