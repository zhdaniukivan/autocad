...
class BlockEditorGUI:
    ...
    def create_widgets(self):
        ...
        # ОДНО поле для ширины (применяется к обеим линиям)
        ...
        tk.Label(width_frame, text="Целевая общая ширина (для верхней и нижней линии):",
                 bg='#1e1e1e', fg='white', width=35, anchor='w').pack(side='left')
    ...
    # ---------- Применение ширины к обеим линиям (оптимизированное) ----------
    def apply_width_both(self):
        ...
        blocks = self.current_blocks[:]  # фиксируем список
        self.update_status("📏 Применение ширины к нижней и верхней линии...")
        self.root.update()

        # Применяем без промежуточной регенерации
        success_bottom, new_bottom, msg_bottom = self.handler.apply_target_width_bottom(blocks, target, regen=False)
        success_top, new_top, msg_top = self.handler.apply_target_width_top(blocks, target, regen=False)

        # Одна регенерация и обновление спецэлементов
        ...
        if success_bottom or success_top:
            ...
            messagebox.showinfo("Ничего не изменено", f"{msg_bottom}\n{msg_top}")
            self.update_status("Ширина не изменена")
    ...
    # ---------- Пакетное применение (ширина + высота) с диагностикой ----------
